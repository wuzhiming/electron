...
const setupMethods = (WebViewElement) => {
  ...
  const createNonBlockHandler = function (method) {
    return function (...args) {
      ipcRendererUtils.invoke('ELECTRON_GUEST_VIEW_MANAGER_ASYNC_CALL', getGuestInstanceId(this), method, args)
        .catch(error => {
          throw error
        })
    }
  ...
  const createPromiseHandler = function (method) {
    return function (...args) {
      return ipcRendererUtils.invoke('ELECTRON_GUEST_VIEW_MANAGER_ASYNC_CALL', getGuestInstanceId(this), method, args)
    }
  }
