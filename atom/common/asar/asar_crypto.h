...
// Copy the implementation of CipherBase from node_crypto.h/.cc(referred as node
// implemetatioin) because node implementation is tightly bind with js
// invocation. And it is not easy to invoke js codes from c++ because this class
// is invoked when initializing environment.
//
// If we can easily construct v8::Local<v8::Object>, then we can use node
// implementation. But i met crash issue when creating v8::Local<v8::Object>
// like this in DecryptHeader() in archive.cc:
//
...
class CipherBase {
 public:
  // Decrypt indata can copy the decrypted data to `indata`, it supposed the
  // length of decrypted data is the same as non-decrypted. It will create a new
  // CipherBase every time, so it is used in the situation that all data ready
  // in indata.
