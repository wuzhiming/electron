...
void CipherBase::Init(const char* cipher_type,
  ...
  int key_len = EVP_BytesToKey(cipher_, EVP_md5(), nullptr,
  ...
  EVP_CipherInit_ex(&ctx_, nullptr, nullptr,
                    ...
                    reinterpret_cast<unsigned char*>(iv), kind_ == kCipher);
  initialised_ = true;
}
...
void CipherBase::InitIv(const char* cipher_type,
  ...
  EVP_CipherInit_ex(
      &ctx_, nullptr, nullptr, reinterpret_cast<const unsigned char*>(key),
      reinterpret_cast<const unsigned char*>(iv), kind_ == kCipher);
  initialised_ = true;
}
...
bool CipherBase::SetAAD(const char* data, unsigned int len) {
  if (!initialised_ || !IsAuthenticatedMode())
    return false;
  int outlen;
  if (!EVP_CipherUpdate(&ctx_, nullptr, &outlen,
...
bool CipherBase::Update(const char* data,
  ...
  *out = new unsigned char[*out_len];
  return EVP_CipherUpdate(&ctx_, *out, out_len,
                          reinterpret_cast<const unsigned char*>(data), len);
}
  ...
  *out = new unsigned char[EVP_CIPHER_CTX_block_size(&ctx_)];
  ...
  if (r && kind_ == kCipher) {
    ...
    if (IsAuthenticatedMode()) {
      ...
      EVP_CIPHER_CTX_ctrl(&ctx_, EVP_CTRL_GCM_GET_TAG, auth_tag_len_,
                          reinterpret_cast<unsigned char*>(auth_tag_));
    }
  }
