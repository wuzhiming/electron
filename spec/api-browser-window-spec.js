  ...
  describe('"webPreferences" option', () => {
    ...
    describe('"sandbox" option', () => {
      ...
      it('validates process APIs access in sandboxed renderer', (done) => {
        ipcMain.once('answer', function (event, test) {
          expect(test.crash).to.equal('function')
          expect(test.hang).to.equal('function')
          expect(test.getCreationTime).to.be.a('number')
          expect(test.getHeapStatistics).to.be.an('object')
          expect(test.getProcessMemoryInfo).to.be.an('object')
          expect(test.getSystemMemoryInfo).to.be.an('object')
          expect(test.getSystemVersion).to.be.a('string')
          expect(test.getCPUUsage).to.be.an('object')
          expect(test.getIOCounters).to.be.an('object')
